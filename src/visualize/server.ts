...
import { getVendorAsset } from "./vendor.js";
...
// Server assets sit locally under /vendor/* for full offline operation - no external CDN.
const CSP = [
  "default-src 'none'",
  "script-src 'self'",
...
export async function runVisualizeServer(
  ...
  const clientLibJs = await readAsset("./client-lib.js");
...
export function createRequestHandler(
  ...
  return (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (url.startsWith("/vendor/")) {
      const vendorAsset = getVendorAsset(url);
      if (vendorAsset) {
        res.writeHead(200, { "content-type": vendorAsset.contentType });
        res.end(vendorAsset.content);
        return;
      }
    }
